...
let transcriptSourceCache = new Map();
let transcriptPlayerSourceCache = new Map();
let playerCaptionAlignedCache = new Map();
const playerCaptionAlignmentFailures = new Set();
const PLAYER_CAPTION_ALIGNMENT_VERSION = "v1";
let currentTranscriptNeedsPunctuationRestore = false;
let transcriptCleanupPromise = null;
let transcriptCleanupAttempted = false;
...
const PLAYER_CAPTION_LIMITS = Object.freeze({
  minChars: 20,
  idealChars: 40,
  maxChars: 76,
  maxCjkChars: 34,
});
let playerCaptionsEnabled = false;
let playerCaptionSourceSignature = "";
let playerCaptionUpdateTimer = null;
let playerCaptionPrefetchInterval = null;
const pendingPlayerCaptionTranslations = new Map();
const PLAYER_CAPTION_ENABLED_KEY = "ytd_player_captions_enabled";
const PLAYER_CAPTION_FONT_SCALE_KEY = "ytd_player_caption_font_scale";
const PLAYER_CAPTION_FONT_SCALES = Object.freeze([
  0.1,
  0.25,
  0.5,
  0.75,
...
function updatePlayerCaptionToggleControl() {
  const button = document.getElementById("playerCaptionToggle");
  if (!button) return;
  button.classList.toggle("active", playerCaptionsEnabled);
  button.setAttribute("aria-pressed", String(playerCaptionsEnabled));
  button.title = playerCaptionsEnabled
    ? "Hide player subtitles"
    : "Show player subtitles";
}

function updatePlayerCaptionControlsVisibility() {
  const showResults =
    document.getElementById("resultsState")?.style.display === "block";
  const toggle = document.getElementById("playerCaptionToggle");
  const scale = document.getElementById("captionScaleControl");
  if (toggle) toggle.style.display = showResults ? "inline-flex" : "none";
  if (scale) {
    scale.style.display =
      showResults && playerCaptionsEnabled ? "inline-flex" : "none";
  }
}

async function loadPlayerCaptionsEnabled() {
  try {
    const stored = await chrome.storage.local.get(PLAYER_CAPTION_ENABLED_KEY);
    playerCaptionsEnabled = stored?.[PLAYER_CAPTION_ENABLED_KEY] === true;
  } catch {
    playerCaptionsEnabled = false;
  }
  updatePlayerCaptionToggleControl();
  updatePlayerCaptionControlsVisibility();
  return playerCaptionsEnabled;
}

async function handlePlayerCaptionsEnabledChange(enabled) {
  playerCaptionsEnabled = enabled === true;
  updatePlayerCaptionToggleControl();
  updatePlayerCaptionControlsVisibility();
  await chrome.storage.local.set({
    [PLAYER_CAPTION_ENABLED_KEY]: playerCaptionsEnabled,
  });
  await syncPlayerCaptions({
    clear: !playerCaptionsEnabled,
    force: true,
  });
}

function normalizePlayerCaptionFontScale(value) {
  const scale = Number(value);
  return PLAYER_CAPTION_FONT_SCALES.includes(scale) ? scale : 1;
}

function updatePlayerCaptionFontScaleControl(scale = playerCaptionFontScale) {
  const select = document.getElementById("captionScaleSelect");
  if (select) select.value = String(normalizePlayerCaptionFontScale(scale));
}

async function loadPlayerCaptionFontScale() {
  try {
    const stored = await chrome.storage.local.get(
      PLAYER_CAPTION_FONT_SCALE_KEY,
    );
    playerCaptionFontScale = normalizePlayerCaptionFontScale(
      stored?.[PLAYER_CAPTION_FONT_SCALE_KEY],
    );
  } catch {
    playerCaptionFontScale = 1;
  }
  updatePlayerCaptionFontScaleControl();
  return playerCaptionFontScale;
}

async function applyPlayerCaptionFontScale() {
  await sendPlayerCaptionMessage({
    action: "setPlayerBilingualCaptionScale",
    scale: playerCaptionFontScale,
  });
}

async function handlePlayerCaptionFontScaleChange(value) {
  playerCaptionFontScale = normalizePlayerCaptionFontScale(value);
  updatePlayerCaptionFontScaleControl();
  await chrome.storage.local.set({
    [PLAYER_CAPTION_FONT_SCALE_KEY]: playerCaptionFontScale,
  });
  await applyPlayerCaptionFontScale();
}

function getPlayerCaptionMaxChars(text, limits = PLAYER_CAPTION_LIMITS) {
  const source = String(text || "");
  const cjkCount = (source.match(/[\u3400-\u9fff]/g) || []).length;
  const cjkRatio = source.length ? cjkCount / source.length : 0;
  return cjkRatio >= 0.3
    ? Math.min(limits.maxChars, limits.maxCjkChars)
    : limits.maxChars;
}

function transcriptPlayerSourceCacheKey(start) {
  return `${currentVideoId}:source:player-entry:${Math.round(Number(start) * 1000)}`;
}

function playerCaptionAlignmentUnitId(captionId, semanticSegmentId) {
  return `${captionId}::${semanticSegmentId}`;
}

function playerCaptionAlignedCacheKey(
  videoId,
  semanticSegmentId,
  displayUnitId,
) {
  return `${videoId}:${PLAYER_CAPTION_ALIGNMENT_VERSION}:${semanticSegmentId}:${displayUnitId}`;
}

function playerCaptionAlignmentFailureKey(videoId, semanticSegmentId) {
  return `${videoId}:${PLAYER_CAPTION_ALIGNMENT_VERSION}:${semanticSegmentId}`;
}

function getPlayerCaptionSemanticSegments() {
  return getActiveTranscriptSegments().map((segment) => {
    const displaySegment = getTranscriptDisplaySegment(segment);
    return {
      ...displaySegment,
      translation:
...
function getPlayerCaptionSourceEntries() {
  if (!Array.isArray(currentTranscript)) return [];

  return currentTranscript.map((entry) => {
    const cachedSource = transcriptPlayerSourceCache.get(
      transcriptPlayerSourceCacheKey(entry?.start),
    );
    return cachedSource
      ? { ...entry, text: cachedSource }
      : { ...entry };
  });
}

function annotatePlayerCaptionSemanticIndexes(entries, semanticSegments) {
  if (!Array.isArray(entries) || !entries.length) return [];
  if (!Array.isArray(semanticSegments) || !semanticSegments.length) {
    return entries.map((entry) => ({ ...entry, semanticIndex: -1 }));
  }
  ...
  return entries.map((entry) => {
    ...
    while (
      semanticIndex + 1 < semanticSegments.length &&
      start >= Number(semanticSegments[semanticIndex + 1]?.start)
    ) {
      semanticIndex += 1;
    }
    return { ...entry, semanticIndex };
  });
}

function findTranslationBoundary(text, index, direction) {
  const source = String(text || "");
  if (!source) return 0;
  ...
  for (let distance = 0; distance <= maxDistance; distance += 1) {
    const candidate = safeIndex + distance * direction;
    if (candidate <= 0 || candidate >= source.length) {
      return Math.max(0, Math.min(source.length, candidate));
    }
    if (boundaryPattern.test(source[candidate - 1])) return candidate;
  }
  return safeIndex;
}

function slicePlayerCaptionTranslation(sourceText, translation, targetText) {
  const translated = normalizeCaptionText(translation);
  const sourceKey = normalizePlayerCaptionSourceText(sourceText);
  const targetKey = normalizePlayerCaptionSourceText(targetText);
  if (!translated || !sourceKey || !targetKey) return "";
  if (sourceKey === targetKey) return translated;

  const sourceOffset = sourceKey.indexOf(targetKey);
  if (sourceOffset < 0) return translated;
  const startRatio = sourceOffset / sourceKey.length;
  const endRatio = (sourceOffset + targetKey.length) / sourceKey.length;
  if (startRatio === 0 && endRatio === 1) return translated;

  const rawStart = Math.floor(translated.length * startRatio);
  const rawEnd = Math.ceil(translated.length * endRatio);
  const start = findTranslationBoundary(translated, rawStart, -1);
  const end = findTranslationBoundary(translated, rawEnd, 1);
  return normalizeCaptionText(translated.slice(start, Math.max(start, end))) ||
    translated;
}

function slicePlayerCaptionSourceText(sourceText, targetText) {
  const source = normalizeCaptionText(sourceText);
  const sourceKey = normalizePlayerCaptionSourceText(source);
  const targetKey = normalizePlayerCaptionSourceText(targetText);
  if (!source || !sourceKey || !targetKey) return "";
  ...
  const startRatio = sourceOffset / sourceKey.length;
  const endRatio = (sourceOffset + targetKey.length) / sourceKey.length;
  const rawStart = Math.floor(source.length * startRatio);
  const rawEnd = Math.ceil(source.length * endRatio);
  const start = findTranslationBoundary(source, rawStart, -1);
  const end = findTranslationBoundary(source, rawEnd, 1);
  return normalizeCaptionText(source.slice(start, Math.max(start, end)));
}

function buildPlayerCaptionSegmentsFromEntries(entries, limits = PLAYER_CAPTION_LIMITS) {
  const groups = [];
  let current = null;
  ...
  entries.forEach((entry, index) => {
    ...
    const nextStart = Number(entries[index + 1]?.start);
    const fallbackEnd =
      Number.isFinite(nextStart) && nextStart > start
        ? nextStart
        : start + Math.max(1, text.length / 12);
    const end =
      duration > 0
        ? start + duration
        : Math.max(start + 0.05, fallbackEnd);
    const semanticIndex = Number.isInteger(entry?.semanticIndex)
      ? entry.semanticIndex
      : -1;

    if (!current) {
      current = { start, end, text, parts: [] };
    } else {
      const candidate = normalizeCaptionText(`${current.text} ${text}`);
      const limit = getPlayerCaptionMaxChars(candidate, limits);
    ...
    current.parts.push({ semanticIndex, text });
    const atSentenceEnd =
      /[.!?。！？]["')\]”’）】」』]*$/.test(text);
    if (
      atSentenceEnd ||
      current.text.length >= getPlayerCaptionMaxChars(current.text, limits)
    ) {
      flush();
    }
  });
  ...
  return groups.map((caption, index) => ({
    id: `player-segment-${index}-${Math.round(caption.start * 1000)}`,
    start: caption.start,
    end: caption.end,
    text: caption.text,
    timingEstimated: false,
    parts: caption.parts.map((part) => ({ ...part })),
  }));
}

function normalizePlayerCaptionSourceText(text) {
  return String(text || "")
    .normalize("NFKC")
    .toLocaleLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, "");
...
function buildPlayerCaptionSourceEntries(semanticSegments) {
  return annotatePlayerCaptionSemanticIndexes(
    getPlayerCaptionSourceEntries(),
    semanticSegments,
  ).map((entry) => {
    const segment = semanticSegments[entry.semanticIndex];
    if (!segment) return entry;
    const restoredText = slicePlayerCaptionSourceText(
      segment.text,
      entry.text,
    );
    return restoredText ? { ...entry, text: restoredText } : entry;
  });
}

function buildPlayerCaptionSourcesWithoutTranslations(semanticSegments) {
  return buildPlayerCaptionSegmentsFromEntries(
    buildPlayerCaptionSourceEntries(semanticSegments),
  );
}

function getPlayerCaptionAlignmentUnitsBySegment(semanticSegments) {
  const unitsBySegment = new Map();
  buildPlayerCaptionSourcesWithoutTranslations(semanticSegments).forEach(
    (caption) => {
      const partsBySegment = new Map();
      (caption.parts || []).forEach((part) => {
        if (!Number.isInteger(part.semanticIndex) || part.semanticIndex < 0) {
          return;
        }
        const existing = partsBySegment.get(part.semanticIndex) || [];
        existing.push(part.text);
        partsBySegment.set(part.semanticIndex, existing);
      });

      partsBySegment.forEach((sourceParts, semanticIndex) => {
        const segment = semanticSegments[semanticIndex];
        if (!segment) return;
        const id = playerCaptionAlignmentUnitId(caption.id, segment.id);
        const text = normalizeCaptionText(sourceParts.join(" "));
        if (!text) return;
        const units = unitsBySegment.get(segment.id) || [];
        units.push({ id, text });
        unitsBySegment.set(segment.id, units);
      });
    },
  );
  return unitsBySegment;
}

function getPlayerCaptionTranslation(caption, semanticSegments) {
  const partsBySegment = new Map();
  (caption.parts || []).forEach((part) => {
    if (!Number.isInteger(part.semanticIndex) || part.semanticIndex < 0) return;
    const existing = partsBySegment.get(part.semanticIndex) || [];
    existing.push(part.text);
    partsBySegment.set(part.semanticIndex, existing);
  });

  return Array.from(partsBySegment.entries())
    .sort(([left], [right]) => left - right)
    .map(([semanticIndex, sourceParts]) => {
      const segment = semanticSegments[semanticIndex];
      if (!segment) return "";
      const displayUnitId = playerCaptionAlignmentUnitId(
        caption.id,
        segment.id,
      );
      const aligned = playerCaptionAlignedCache.get(
        playerCaptionAlignedCacheKey(
          currentVideoId,
          segment.id,
          displayUnitId,
        ),
      ...
      if (
        playerCaptionAlignmentFailures.has(
          playerCaptionAlignmentFailureKey(currentVideoId, segment.id),
        )
      ) {
        return slicePlayerCaptionTranslation(
          segment.text,
          segment.translation,
          sourceParts.join(" "),
        );
...
function groupPlayerCaptionEntries(entries) {
  return buildPlayerCaptionSegmentsFromEntries(entries);
}

function buildPlayerBilingualCaptionSegments(entries, limits) {
  return buildPlayerCaptionSegmentsFromEntries(entries, limits);
}

function getPlayerCaptionSources() {
  const semanticSegments = getPlayerCaptionSemanticSegments();
  return buildPlayerCaptionSourcesWithoutTranslations(semanticSegments).map(
    (caption) => ({
      id: caption.id,
      start: caption.start,
      end: caption.end,
      timingEstimated: false,
      text: caption.text,
      translation: getPlayerCaptionTranslation(caption, semanticSegments),
      semanticIndexes: Array.from(
        new Set(
          (caption.parts || [])
            .map((part) => part.semanticIndex)
...
function getPlayerBilingualCaptionSources() {
  return getPlayerCaptionSources();
}

async function sendPlayerCaptionMessage(payload) {
  if (!youtubeTabId) return;
  try {
    await chrome.tabs.sendMessage(youtubeTabId, payload);
  } catch {
    // The YouTube tab may be navigating or the content script may be reloading.
  }
}

function getPlayerCaptionSignature(captions, displayMode) {
  return [
    currentVideoId || "",
    displayMode,
    captions.length,
...
async function syncPlayerCaptions({
  clear = false,
  force = false,
} = {}) {
  if (
    clear ||
    !playerCaptionsEnabled ||
    !currentTranscript?.length
  ) {
    playerCaptionSourceSignature = "";
    clearTimeout(playerCaptionUpdateTimer);
    playerCaptionUpdateTimer = null;
    pendingPlayerCaptionTranslations.clear();
    stopPlayerCaptionTranslationPrefetch();
    await sendPlayerCaptionMessage({ action: "clearPlayerBilingualCaptions" });
    return;
  }

  const captions = getPlayerCaptionSources();
  if (!captions.length) {
    playerCaptionSourceSignature = "";
    stopPlayerCaptionTranslationPrefetch();
    await sendPlayerCaptionMessage({ action: "clearPlayerBilingualCaptions" });
  ...
  const displayMode = currentTranscriptMode;
  const signature = getPlayerCaptionSignature(captions, displayMode);
  if (!force && signature === playerCaptionSourceSignature) {
    if (displayMode !== "original") {
      startPlayerCaptionTranslationPrefetch();
    }
    return;
  }
  playerCaptionSourceSignature = signature;
...
async function syncPlayerBilingualCaptions(options) {
  return syncPlayerCaptions(options);
}

function queuePlayerCaptionTranslations(captions) {
  if (
    !playerCaptionsEnabled ||
  ...
  captions.forEach((caption) => {
    ...
    pendingPlayerCaptionTranslations.set(caption.id, { ...caption });
  });
  clearTimeout(playerCaptionUpdateTimer);
  playerCaptionUpdateTimer = setTimeout(() => {
    playerCaptionUpdateTimer = null;
    const translations = Array.from(
      pendingPlayerCaptionTranslations,
      ([id, value]) => ({
        id,
        ...
        timingEstimated: value.timingEstimated === true,
      }),
    );
    pendingPlayerCaptionTranslations.clear();
    void sendPlayerCaptionMessage({
...
function queuePlayerCaptionUpdateForSegment(segment) {
  if (!playerCaptionsEnabled || currentTranscriptMode === "original") return;
  const semanticIndex = getActiveTranscriptSegments().findIndex(
    (candidate) => candidate.id === segment?.id,
  );
  const captions = getPlayerCaptionSources().filter(
    (caption) =>
      semanticIndex < 0 ||
      caption.semanticIndexes?.includes(semanticIndex),
  );
  queuePlayerCaptionTranslations(captions);
}

function findTranscriptIndexAtTime(segments, currentSeconds) {
  let activeIndex = -1;

  for (let index = 0; index < segments.length; index += 1) {
    if (Number(segments[index].start) > currentSeconds) break;
    activeIndex = index;
  }
...
async function prefetchPlayerCaptionTranslations() {
  if (
    !playerCaptionsEnabled ||
    currentTranscriptMode === "original" ||
    !currentTranscript?.length ||
  ...
  try {
    ...
    const currentSeconds = Number(response?.currentTime) || 0;
    const playerSegments = getActiveTranscriptSegments();
    const activePlayerIndex = findTranscriptIndexAtTime(
      playerSegments,
      currentSeconds,
    );
    if (activePlayerIndex < 0) return;

    const playerQueue = getOrCreateTranscriptTranslationQueue(
      playerSegments,
      translationGeneration,
      currentVideoId,
      currentTranscriptMode,
    );
    const finalPlayerIndex = Math.min(
      activePlayerIndex + 5,
      playerSegments.length - 1,
    );
    for (
      let index = activePlayerIndex;
      index <= finalPlayerIndex;
      index += 1
    ) {
      playerQueue.enqueue(index);
    }
  } catch {
    // Playback tracking is best-effort while the YouTube tab is changing.
  }
}

function startPlayerCaptionTranslationPrefetch() {
  if (playerCaptionPrefetchInterval) return;
  void prefetchPlayerCaptionTranslations();
  playerCaptionPrefetchInterval = setInterval(
    () => void prefetchPlayerCaptionTranslations(),
    750,
  );
}

function stopPlayerCaptionTranslationPrefetch() {
  clearInterval(playerCaptionPrefetchInterval);
  playerCaptionPrefetchInterval = null;
}
...
function transcriptNeedsPunctuationRestore(entries, plainText = "") {
  if (
    typeof YTD_SETTINGS === "undefined" ||
    typeof YTD_SETTINGS.needsPunctuationRestore !== "function"
  ) {
    return false;
  }
  ...
  const sourceText =
    ...
    (Array.isArray(entries)
      ...
      : "");
  return YTD_SETTINGS.needsPunctuationRestore(sourceText);
}
...
function groupTranscriptEntries(entries, limits = TRANSCRIPT_SEGMENT_LIMITS) {
  ...
  entries.forEach((entry, entryIndex) => {
    ...
    sentenceParts.forEach((sentencePart) => {
      ...
      oversizedParts.forEach((part, partIndex) => {
        const startRatio = text.length
          ? Math.min(1, consumedChars / text.length)
          : 0;
        const endRatio = text.length
  ...
  const flush = () => {
    ...
    grouped.push({
      ...
      start: current.start,
      end: current.end > current.start ? current.end : null,
      timingEstimated: current.timingEstimated,
      text,
      texts: [text],
    });
    current = null;
  ...
  pieces.forEach((piece) => {
    if (!current) {
      current = {
        ...
        timingEstimated: piece.timingEstimated,
        text: "",
      };
    } else {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  setupEventListeners();
  await loadPlayerCaptionsEnabled();
  await loadPlayerCaptionFontScale();
...
function setupEventListeners() {
  ...
  document
    .getElementById("captionScaleSelect")
    ?.addEventListener("change", (event) => {
      void handlePlayerCaptionFontScaleChange(event.target.value);
    });
  document
    .getElementById("playerCaptionToggle")
    ?.addEventListener("click", () => {
      void handlePlayerCaptionsEnabledChange(!playerCaptionsEnabled);
    });
  ...
  window.addEventListener("pagehide", () => {
    void saveCurrentTranscriptViewState();
    void syncPlayerBilingualCaptions({ clear: true });
  });
...
async function checkCurrentTab() {
  try {
    ...
    youtubeTabId = tab.id;
    void applyPlayerCaptionFontScale();
...
function extractVideoId(url) {
  try {
    ...
    if (isYouTubeHost && urlObj.searchParams.has("v")) {
      return normalizeId(urlObj.searchParams.get("v"));
    }

    if (urlObj.hostname === "youtu.be") {
      return normalizeId(urlObj.pathname.slice(1));
    }

    if (urlObj.pathname.startsWith("/embed/")) {
      return normalizeId(urlObj.pathname.split("/")[2]);
    }
...
async function startDigest(videoId, videoUrl) {
  ...
  if (videoId === currentVideoId && currentAnalysis) {
    showState("results");
    void syncPlayerBilingualCaptions({ force: true });
    return;
  }
  ...
  if (cached) {
    ...
    currentTranscriptNeedsPunctuationRestore =
      transcriptNeedsPunctuationRestore(
        currentTranscript,
    ...
    if (cached.sourceCache) {
      for (const [key, value] of Object.entries(cached.sourceCache)) {
        transcriptSourceCache.set(key, value);
      }
      refreshCurrentTranscriptPlainText();
    }
    if (cached.playerSourceCache) {
      for (const [key, value] of Object.entries(cached.playerSourceCache)) {
        transcriptPlayerSourceCache.set(key, value);
      }
    }
    if (cached.sourceCache) {
      if (hasCompleteRestoredSourceCache()) {
        currentTranscriptNeedsPunctuationRestore = false;
      }
    }
    ...
    if (cached.playerCaptionAlignmentCache) {
      for (const [key, value] of Object.entries(
        cached.playerCaptionAlignmentCache,
      )) {
        playerCaptionAlignedCache.set(key, value);
      }
    }
    ...
    if (currentTranscriptMode !== "original") {
      await restoreTranscriptPunctuation();
    }
  ...
  currentTranscriptNeedsPunctuationRestore = false;
  transcriptCleanupPromise = null;
  transcriptCleanupAttempted = false;
  isAnalysisLoading = false;
  ...
  currentTranscriptNeedsPunctuationRestore =
    Boolean(transcriptResult.needsPunctuationRestore) ||
    transcriptNeedsPunctuationRestore(
      currentTranscript,
      currentTranscriptText,
    );
...
function renderTranscript() {
  ...
  grouped.forEach((group) => {
    const displayGroup = getTranscriptDisplaySegment(group);
    const div = document.createElement("div");
    div.className = "transcript-entry";
    div.dataset.seconds = group.start;
    div.dataset.segmentId = group.id;

    const timestamp = formatTranscriptTimestamp(group.start);

    div.innerHTML = `
      <span class="transcript-time">${timestamp}</span>
      <span class="transcript-text">${renderSubtitleInlineMarkup(displayGroup.text)}</span>
    `;
  ...
  void syncPlayerBilingualCaptions();
...
function playNote(note) {
  if (note.videoId && note.videoId === currentVideoId) {
    ...
    const safeUrl = getSafeTimestampedNoteUrl(note);
...
async function saveToCache(videoId) {
  ...
  try {
    ...
    const sourceCacheForVideo = {};
    for (const [key, value] of transcriptSourceCache.entries()) {
      if (key.startsWith(`${videoId}:`)) {
        sourceCacheForVideo[key] = value;
      }
    }
    const playerSourceCacheForVideo = {};
    for (const [key, value] of transcriptPlayerSourceCache.entries()) {
      if (key.startsWith(`${videoId}:`)) {
        playerSourceCacheForVideo[key] = value;
      }
    }
    ...
    const playerCaptionAlignmentCacheForVideo = {};
    for (const [key, value] of playerCaptionAlignedCache.entries()) {
      if (key.startsWith(`${videoId}:`)) {
        playerCaptionAlignmentCacheForVideo[key] = value;
      }
    }
    const cacheData = {
      ...
      sourceCache: sourceCacheForVideo,
      playerSourceCache: playerSourceCacheForVideo,
      interfaceCache: interfaceCacheForVideo,
      playerCaptionAlignmentCache: playerCaptionAlignmentCacheForVideo,
      timestamp: Date.now(),
    };
  ...
  notes.forEach((note, index) => {
    ...
    noteEl.innerHTML = `
      <div class="note-header">
        <span class="note-timestamp" data-url="${escapeHtml(safeTimestampedUrl)}" data-seconds="${Number(note.timestampSeconds) || 0}">${escapeHtml(note.timestamp)}</span>
      ...
      <div class="note-actions">
        ...
        <button class="note-action-btn note-copy-link" data-url="${escapeHtml(safeTimestampedUrl)}">Copy timestamp</button>
...
function startPlaybackTracking() {
  ...
  // Poll video time frequently enough that the row highlight tracks the
  // player caption boundary instead of visibly lagging behind it.
  autoScrollInterval = setInterval(() => playbackTrackingTick(), 250);
...
function transcriptSourceCacheKey(segment) {
  return `${currentVideoId}:source:${segment.id}`;
}

function getTranscriptDisplaySegment(segment) {
  const restoredSource = transcriptSourceCache.get(
    transcriptSourceCacheKey(segment),
  );
  return restoredSource ? { ...segment, text: restoredSource } : segment;
}

function refreshCurrentTranscriptPlainText() {
  if (!currentTranscript?.length) return;
  currentTranscriptText = getActiveTranscriptSegments()
    .map((segment) => getTranscriptDisplaySegment(segment).text)
    .join(" ")
    .trim();
}

function hasCompleteRestoredSourceCache() {
  const segments = getActiveTranscriptSegments();
  const playerEntries = Array.isArray(currentTranscript)
    ? currentTranscript
    : [];
  return (
    segments.length > 0 &&
    segments.every((segment) => {
      const cachedSource = transcriptSourceCache.get(
        transcriptSourceCacheKey(segment),
      );
      return (
        typeof cachedSource === "string" &&
        cachedSource.trim() &&
        !transcriptNeedsPunctuationRestore([], cachedSource)
      );
    }) &&
    (!currentTranscriptNeedsPunctuationRestore ||
      playerEntries.every((entry) =>
        transcriptPlayerSourceCache.has(
          transcriptPlayerSourceCacheKey(entry?.start),
        ),
      ))
  );
...
function applyCleanedTranscriptSources(cleanedEntries) {
  if (!Array.isArray(cleanedEntries) || !cleanedEntries.length) return false;

  const cleanedBySecond = new Map();
  cleanedEntries.forEach((entry) => {
    const second = Number(entry?.start);
    const text = normalizeCaptionText(entry?.text);
    if (!Number.isFinite(second) || !text) return;
    const existing = cleanedBySecond.get(second);
    cleanedBySecond.set(
      second,
      existing ? normalizeCaptionText(`${existing} ${text}`) : text,
    );
  });

  (currentTranscript || []).forEach((entry) => {
    const rawStart = Number(entry?.start);
    const cleanedText = cleanedBySecond.get(Math.floor(rawStart));
    if (Number.isFinite(rawStart) && cleanedText) {
      transcriptPlayerSourceCache.set(
        transcriptPlayerSourceCacheKey(rawStart),
        cleanedText,
      );
    }
  });
  ...
  segments.forEach((segment, index) => {
    const nextStart =
      index + 1 < segments.length
        ...
        : Infinity;
    const rawEntries = (currentTranscript || []).filter(
      (entry) =>
        Number(entry.start) >= Number(segment.start) &&
        Number(entry.start) < nextStart,
    );
    const cleanedText = cleanedEntries
      .filter((entry) =>
        rawEntries.some(
          (rawEntry) =>
            Math.floor(Number(rawEntry.start)) === Number(entry.start),
        ),
      )
      .map((entry) => entry.text)
      .join(" ");
    const normalized = normalizeCaptionText(cleanedText) || segment.text;
    transcriptSourceCache.set(transcriptSourceCacheKey(segment), normalized);
    if (normalized !== segment.text) restoredCount += 1;
  });
...
async function restoreTranscriptPunctuation() {
  if (
    !currentTranscriptNeedsPunctuationRestore ||
    !currentTranscript?.length ||
    !currentTranscriptTimestamped ||
    transcriptCleanupAttempted
  ) {
    return false;
  }
  if (transcriptCleanupPromise) return transcriptCleanupPromise;

  transcriptCleanupAttempted = true;
  transcriptCleanupPromise = (async () => {
    setTranslatingSpinner(true);
    try {
      const result = await chrome.runtime.sendMessage({
      ...
      if (!result?.success || !Array.isArray(result.enhancedTranscript)) {
        console.warn(
          "[YouTube Digest] Transcript punctuation cleanup failed:",
          result?.error || "Unknown error",
        );
        return false;
      }

      const changed = applyCleanedTranscriptSources(result.enhancedTranscript);
      await updateCache();
      await syncPlayerBilingualCaptions({ force: true });
      if (changed) {
        if (currentTranscriptMode === "original") {
          renderTranscript();
        }
      ...
      console.warn("[YouTube Digest] Transcript punctuation cleanup error:", error);
      return false;
    } finally {
      setTranslatingSpinner(false);
    }
  ...
  return transcriptCleanupPromise;
}
...
async function handleDisplayLanguageModeChange(mode) {
  ...
  if (mode !== "original") {
    interfaceTranslationFailures.clear();
    playerCaptionAlignmentFailures.clear();
  }
  translationGeneration += 1;
  ...
  if (playerCaptionsEnabled) {
    void syncPlayerCaptions({ force: true });
  }
  const activeTabName =
...
function renderTranscriptModeRows(segments, mode) {
  ...
  segments.forEach((segment, index) => {
    ...
    div.innerHTML = `
      <span class="transcript-time">${timestamp}</span>
      ${renderTranscriptSegmentContent(displaySegment, mode, cached, "")}
    `;
    div.addEventListener("click", (event) =>
...
function alignTranslatedSegmentBatch(sourceSegments, responseSegments) {
    ...
    responseSegments.forEach((item) => {
      ...
      if (text && !translatedById.has(item.id)) {
        translatedById.set(item.id, {
          ...
          displayUnits: Array.isArray(item.displayUnits)
            ? item.displayUnits
            : [],
        });
      }
  ...
  return sourceSegments.map((segment) => {
    ...
    const requestedUnits = Array.isArray(segment.displayUnits)
      ? segment.displayUnits
      : [];
    const translatedUnitMap = new Map(
      (translated?.displayUnits || [])
        .filter(
          (unit) =>
            unit &&
    ...
    const displayUnits = requestedUnits
      .map((unit) => ({
        id: unit.id,
        text: translatedUnitMap.get(unit.id) || "",
      }))
    ...
    return {
      ...
      displayUnits,
      error: translated ? "" : "Translation unavailable.",
    };
  });
}
...
function updateTranslatedRow(segment, index, alignedItem, generation) {
  ...
  if (alignedItem.text) {
    ...
    const restoredSource =
      alignedItem.source && alignedItem.source !== segment.text
        ? alignedItem.source
        : "";
    if (restoredSource) {
      transcriptSourceCache.set(
        transcriptSourceCacheKey(segment),
        restoredSource,
      );
    }
    const requestedDisplayUnits = Array.isArray(segment.displayUnits)
      ? segment.displayUnits
      : [];
    const receivedDisplayUnits = Array.isArray(alignedItem.displayUnits)
      ? alignedItem.displayUnits
      : [];
    if (
      requestedDisplayUnits.length > 0 &&
      receivedDisplayUnits.length === requestedDisplayUnits.length
    ) {
      alignedItem.displayUnits.forEach((unit) => {
        if (!unit?.id || !unit.text) return;
        playerCaptionAlignedCache.set(
          playerCaptionAlignedCacheKey(currentVideoId, segment.id, unit.id),
          unit.text,
        );
      });
      playerCaptionAlignmentFailures.delete(
        playerCaptionAlignmentFailureKey(currentVideoId, segment.id),
      );
    } else if (requestedDisplayUnits.length > 0) {
      playerCaptionAlignmentFailures.add(
        playerCaptionAlignmentFailureKey(currentVideoId, segment.id),
      );
    }
    queuePlayerCaptionUpdateForSegment(segment);
  }
...
async function requestTranscriptTranslationBatch(
  ...
  videoId,
  mode,
  alignmentUnitsBySegment = null,
) {
  const sourceBatch = indices.map((index) => {
    const segment = segments[index];
    return {
      ...segment,
      displayUnits: alignmentUnitsBySegment?.get(segment.id) || [],
    };
  });
  setTranslatingSpinner(true);
  try {
    const result = await sendTranslationMessage({
      action: "translateContent",
      content: {
        segments: sourceBatch.map(({ id, text, displayUnits }) => {
          return displayUnits.length
            ? { id, text, displayUnits }
            : { id, text };
        }),
      },
    ...
    if (
      currentTranscriptNeedsPunctuationRestore &&
      hasCompleteRestoredSourceCache()
    ) {
      currentTranscriptNeedsPunctuationRestore = false;
    }
...
function createTranscriptTranslationQueue(
  ...
  const includeAlignment =
    playerCaptionsEnabled && mode !== "original";
  const alignmentUnitsBySegment = includeAlignment
    ? getPlayerCaptionAlignmentUnitsBySegment(
        getPlayerCaptionSemanticSegments(),
      )
    : null;
  ...
  const enqueue = (index, force = false) => {
    ...
    const translationCached = transcriptParagraphCache.has(
      transcriptTranslationCacheKey(segments[index]),
    );
    const sourceReady =
      !currentTranscriptNeedsPunctuationRestore ||
      transcriptSourceCache.has(transcriptSourceCacheKey(segments[index]));
    const requestedUnits =
      alignmentUnitsBySegment?.get(segments[index].id) || [];
    const alignmentReady =
      !requestedUnits.length ||
      requestedUnits.every((unit) =>
        playerCaptionAlignedCache.has(
          playerCaptionAlignedCacheKey(videoId, segments[index].id, unit.id),
        ),
      );
    const alignmentFailed = playerCaptionAlignmentFailures.has(
      playerCaptionAlignmentFailureKey(videoId, segments[index].id),
    );
    if (
      (!force &&
        translationCached &&
        sourceReady &&
        (alignmentReady || alignmentFailed)) ||
      queued.has(index)
    ) {
      return;
    }
  ...
  return { enqueue, generation, videoId, mode, includeAlignment };
}

function getOrCreateTranscriptTranslationQueue(
  segments,
  generation,
  videoId,
  mode,
  ...
  if (
    activeTranslationQueue?.generation === generation &&
    activeTranslationQueue.videoId === videoId &&
    activeTranslationQueue.mode === mode &&
    activeTranslationQueue.includeAlignment ===
      (playerCaptionsEnabled && mode !== "original")
  ) {
    return activeTranslationQueue;
  }
...
async function translateTranscript() {
  ...
  const queue = getOrCreateTranscriptTranslationQueue(
    segments,
    generation,
    videoId,
    mode,
  ...
  transcriptScrollObserver = new IntersectionObserver(
    (observerEntries) => {
      observerEntries
        ...
        .forEach((entry) =>
          queue.enqueue(Number(entry.target.dataset.segmentIndex)),
        );
    },
    {
  ...
  rows.forEach((row, index) => {
    ...
    if (index < TRANSLATION_BATCH_SIZE) queue.enqueue(index);
  });
}
...
globalThis.__YTD_TRANSCRIPT_TESTING__ = {
  ...
  groupPlayerCaptionEntries,
  getPlayerCaptionMaxChars,
  slicePlayerCaptionSourceText,
  normalizePlayerCaptionFontScale,
  loadPlayerCaptionsEnabled,
  formatTranscriptTimestamp,
  buildPlayerBilingualCaptionSegments,

...
const YOUTUBE_CONTENT_ACTIONS = new Set(["openSidePanel", "saveNote"]);
 ...
 * in open YouTube tabs. Their buttons remain in the DOM, but their message
 * channels no longer work until the page is refreshed. Re-inject the current
 * content script after install/update so existing tabs recover immediately.
 */
async function recoverYouTubeContentScripts() {
  try {
    ...
    console.warn(
      "[YouTube Digest BG] Could not refresh YouTube content scripts:",
      error,
    );
  }
}
...
function getComparableSourceWords(text) {
  return (
    String(text || "")
      .toLowerCase()
...
/**
 * Punctuation restoration may change whitespace, capitalization, and
 * punctuation, but it must not add, remove, or replace source words.
 */
function isSafeRestoredSource(candidate, original) {
  const candidateWords = getComparableSourceWords(candidate);
  const originalWords = getComparableSourceWords(original);
  return (
    candidateWords.length > 0 &&
    candidateWords.length === originalWords.length &&
    candidateWords.every((word, index) => word === originalWords[index])
  );
}
  ...
  candidates.forEach((candidate) => {
    ...
    if (text && looksLikeChineseTranslation(text, source.text)) {
      const restoredSource =
        ...
        isSafeRestoredSource(candidate.source.trim(), source.text)

...
segment has a stable `id` and source-language `text`. A segment may also include
`displayUnits`, which are the shorter player-caption portions of that semantic
segment.
...
- If `displayUnits` are present, return one translated display unit for every
  input display-unit ID, in the same order. Each display-unit translation must
  correspond only to that display unit's source text.
- The translated display units, joined in order, must cover the complete
  semantic translation without omitting, duplicating, or reordering meaning.
- For each segment, also return `source`: the same source words in the same
  order with natural capitalization and punctuation restored.
- If the source is a word stream with commas between almost every word, treat
  those commas as ASR token separators, not punctuation. Rebuild sentence
  punctuation from meaning, pauses implied by the text, and neighboring
  segments.
- Example: ASR source "so, what, do, you, do, here, in, Bali" becomes
  "So, what do you do here in Bali?".
- If the source punctuation is already natural, preserve it without rewriting.
- The `source` value must not translate, add, remove, replace, or reorder words.
- When display units are requested, return this shape:
  {"segments":[{"id":"unchanged-id","text":"translated text","source":"source text with restored punctuation","displayUnits":[{"id":"unchanged-display-id","text":"translated display text"}]}]}.
- When no display units are requested, omit the `displayUnits` field.
- Copy every input id and display-unit id exactly. Translate only the `text`
  values. Only restore punctuation and capitalization in the `source` values.

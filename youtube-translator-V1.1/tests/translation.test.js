...
test("extension reload reinjects content scripts into open YouTube tabs", async () => {
  const injected = [];
  ...
  assert.deepEqual(
    injected.map((options) => options.target.tabId),
    [11, 12],
  );
  assert.ok(
    injected.every((options) => options.files.includes("content.js")),
  );
});
...
test("DeepSeek retries one empty transcript JSON response without response_format", async () => {
  ...
  assert.equal(requests[0].max_tokens, 2560);
});

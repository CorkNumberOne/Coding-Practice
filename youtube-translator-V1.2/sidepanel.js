...
async function prefetchPlayerCaptionTranslations() {
  ...
  try {
    ...
    const playerQueue = getOrCreatePlayerCaptionAlignmentQueue(
      playerSegments,
...
function alignDisplayUnitBatch(sourceSegments, responseSegments) {
  const responseById = new Map(
    (Array.isArray(responseSegments) ? responseSegments : [])
      .filter((item) => item && typeof item.id === "string")
      .map((item) => [item.id, item]),
  );

  return sourceSegments.map((segment) => {
    const response = responseById.get(segment.id);
    const responseUnits = new Map(
      (Array.isArray(response?.displayUnits)
        ? response.displayUnits
        : []
      )
        .filter(
          (unit) =>
    ...
    return {
      id: segment.id,
      displayUnits: (segment.displayUnits || [])
        .map((unit) => ({
          id: unit.id,
          text: responseUnits.get(unit.id) || "",
        }))
        .filter((unit) => unit.text),
    };
  });
...
function updateTranslatedRow(segment, index, alignedItem, generation) {
  ...
  if (alignedItem.text) {
    ...
    const restoredSource = normalizeCaptionText(
      alignedItem.source || segment.text,
    );
    transcriptSourceCache.set(
      transcriptSourceCacheKey(segment),
      restoredSource || segment.text,
    );
...
let activePlayerCaptionAlignmentQueue = null;
      ...
      content: {
        segments: sourceBatch.map(({ id, text, displayUnits }) => {
          return {
            id,
            text,
            ...(displayUnits.length ? { displayUnits } : {}),
          };
        }),
      },
...
async function requestPlayerCaptionAlignmentBatch(
  indices,
  segments,
  generation,
  videoId,
  ...
  try {
    ...
    const responseSegments = result?.success
      ? result.translatedContent?.segments
      : [];
    const aligned = alignDisplayUnitBatch(sourceBatch, responseSegments);
    aligned.forEach((item, batchIndex) => {
      const segment = sourceBatch[batchIndex];
      const requestedUnits = segment.displayUnits || [];
      const receivedUnits = item.displayUnits || [];
      if (
        requestedUnits.length > 0 &&
        receivedUnits.length === requestedUnits.length
      ) {
        receivedUnits.forEach((unit) => {
          playerCaptionAlignedCache.set(
            playerCaptionAlignedCacheKey(videoId, segment.id, unit.id),
            unit.text,
          );
        });
        playerCaptionAlignmentFailures.delete(
          playerCaptionAlignmentFailureKey(videoId, segment.id),
        );
      } else {
        playerCaptionAlignmentFailures.add(
          playerCaptionAlignmentFailureKey(videoId, segment.id),
...
function createTranscriptTranslationQueue(
  ...
  requestBatch = requestTranscriptTranslationBatch,
) {
  const queue = [];
  const queued = new Set();
  let processing = false;
  const includeAlignment = forceAlignment === true;
  ...
  const enqueue = (index, force = false) => {
    ...
    const sourceReady = transcriptSourceCache.has(
      transcriptSourceCacheKey(segments[index]),
    );
    const requestedUnits =
      alignmentUnitsBySegment?.get(segments[index].id) || [];
    if (includeAlignment && !requestedUnits.length) return;
    ...
    const cachedForMode = includeAlignment
      ? alignmentReady || alignmentFailed
      : translationCached && (alignmentReady || alignmentFailed);
    if ((!force && sourceReady && cachedForMode) || queued.has(index)) {
      return;
    }
    queue.push(index);
    queued.add(index);
...
function getOrCreateTranscriptTranslationQueue(
  ...
  if (
    ...
    activeTranslationQueue.includeAlignment === false
  ) {
...
function getOrCreatePlayerCaptionAlignmentQueue(
  segments,
  generation,
  videoId,
  mode,
  ...
  if (
    activePlayerCaptionAlignmentQueue?.generation === generation &&
    activePlayerCaptionAlignmentQueue.videoId === videoId &&
    activePlayerCaptionAlignmentQueue.mode === mode &&
    activePlayerCaptionAlignmentQueue.includeAlignment === true
  ) {
    return activePlayerCaptionAlignmentQueue;
  }

  activePlayerCaptionAlignmentQueue = createTranscriptTranslationQueue(
    segments,
    generation,
    videoId,
    mode,
    true,
    requestPlayerCaptionAlignmentBatch,
  );
  return activePlayerCaptionAlignmentQueue;
}

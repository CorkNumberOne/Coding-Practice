...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "setPlayerBilingualCaptions") {
    setPlayerBilingualCaptions(message.captions, message.videoId);
    sendResponse({ success: true });
    return false;
  }

  if (message.action === "updatePlayerBilingualCaptionTranslations") {
...
function setPlayerBilingualCaptions(captions, videoId) {
...
function updatePlayerBilingualCaptionTranslations(translations) {
  ...
  translations.forEach((item) => {
    ...
    if (typeof item.source === "string" && item.source.trim()) {
      const caption = ytdBilingualCaptionSources.find(
        (candidate) => candidate.id === item.id,
      );
      if (caption) caption.text = item.source.trim();
    }
  });
...
function startBilingualCaptionSync() {
  if (ytdBilingualCaptionTimer) return;
  updatePlayerBilingualCaption();
  ytdBilingualCaptionTimer = setInterval(updatePlayerBilingualCaption, 200);
}
...
function nativeYouTubeCaptionsAreVisible() {
  ...
  ).some((segment) => segment.textContent?.trim());
}
...
function findActiveBilingualCaption(currentTime) {
  ...
  for (let index = 0; index < ytdBilingualCaptionSources.length; index += 1) {
    ...
    if (caption.start > currentTime) break;
    activeIndex = index;
  }

  if (activeIndex < 0) return null;
  const caption = ytdBilingualCaptionSources[activeIndex];
  if (caption.end !== null && currentTime >= caption.end) return null;
  return caption;
}
...
function updatePlayerBilingualCaption() {
  ...
  if (!nativeYouTubeCaptionsAreVisible()) {
    overlay.hidden = true;
    player.classList.remove(BILINGUAL_CAPTION_HIDE_CLASS);
    return;
  }
  ...
  const caption = findActiveBilingualCaption(video?.currentTime || 0);
  if (!caption) {
    overlay.hidden = true;
    player.classList.remove(BILINGUAL_CAPTION_HIDE_CLASS);
    return;
  ...
  translation.textContent = translatedText || "Translating...";
  translation.classList.toggle("pending", !translatedText);

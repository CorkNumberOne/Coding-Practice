...
function getPlayerBilingualCaptionSources() {
  return getActiveTranscriptSegments().map((segment, index, segments) => {
...
async function prefetchPlayerCaptionTranslations() {
  ...
  try {
    ...
    const activeIndex = findTranscriptIndexAtTime(segments, currentSeconds);
    ...
    const finalIndex = Math.min(activeIndex + 1, segments.length - 1);
    for (let index = activeIndex; index <= finalIndex; index += 1) {
      queue.enqueue(index);
    }
  } catch {
...
function transcriptNeedsPunctuationRestore(entries, plainText = "") {
  if (
    ...
    typeof YTD_SETTINGS.hasDenseAsrCommaArtifacts !== "function"
  ) {
    return false;
  }
  ...
  const sourceText =
    ...
    (Array.isArray(entries)
      ...
      : "");
  return YTD_SETTINGS.hasDenseAsrCommaArtifacts(sourceText);
}
...
function hasCompleteRestoredSourceCache() {
  ...
  return (
    ...
    segments.every((segment) =>
      transcriptSourceCache.has(transcriptSourceCacheKey(segment)),
    )
  );
}

...
- Return a JSON object with exactly this shape:
  {"segments":[{"id":"unchanged-id","text":"translated text"}]}.
...
Input is a JSON object with 1 to 4 semantic transcript segments. Each segment
has the complete source-language `text` and a `displayUnits` array describing
the shorter player-caption portions of that semantic segment.

```
You are aligning a complete transcript translation to shorter player-caption
units in {langName}. The video is titled "{videoTitle}".

{baseRules}

- Use the complete segment text to understand context, names, pronouns, and terminology.
- Return one translated display unit for every input display-unit ID, in the same order.
- Each display-unit translation must correspond only to that display unit's source text.
- Together, the display units must form a complete, natural translation of the semantic segment.
- Do not omit, duplicate, or reorder meaning.
- Do not return the complete translation or restored source text.
- Return a JSON object with exactly this shape:
  {"segments":[{"id":"unchanged-id","displayUnits":[{"id":"unchanged-display-id","text":"translated display text"}]}]}.
- Copy every segment id and display-unit id exactly.
- Output only valid JSON. No markdown fences, commentary, labels, or extra keys.
```

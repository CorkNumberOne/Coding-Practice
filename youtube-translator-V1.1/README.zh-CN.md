...
- 选择双语模式后，播放器中正在显示的原生字幕会变成按时间同步的中英双语字幕。
- 当 Supadata 返回被切碎的自动字幕时，恢复自然的标点和句子边界。
- 只翻译当前可见的 Overview 和 Notes 内容，并通过小批次渐进显示和缓存结果。
- 选中字幕后，可以查看中英双语内容讲解，或保存带时间戳的笔记。

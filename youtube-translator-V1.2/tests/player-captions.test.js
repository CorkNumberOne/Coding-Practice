...
test("player captions reuse semantic translations and prefetch five segments ahead", () => {
  ...
  assert.doesNotMatch(js, /await restoreTranscriptPunctuation\(\)/);
  assert.doesNotMatch(
    js,
    /currentTranscriptMode !== "original"\)\s*\{\s*await restoreTranscriptPunctuation/,
  );
...
test("translation prompt returns only translated text", () => {
  const prompt = read("prompts/translation.md");
  const transcriptPrompt = prompt.slice(
    prompt.indexOf("## Transcript batch translation"),
    prompt.indexOf("## Player caption alignment"),
  );
  ...
  assert.doesNotMatch(transcriptPrompt, /"source"/);
  assert.doesNotMatch(transcriptPrompt, /displayUnits/);
});

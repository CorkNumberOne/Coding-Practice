...
test("preview storage never persists API keys or private digest data", async () => {
  const localStorage = createLocalStorage();
  localStorage.setItem(
    "youtubeDigestPreview:ytd_settings",
    JSON.stringify({ aiApiKey: "legacy-secret" }),
  );
  const storage = options.createStorageAdapter(
    null,
    localStorage,
    (key) =>
      key === "ytd_settings" ||
      key === "ytd_notes" ||
      String(key).startsWith("digest_"),
  );
  assert.equal(
    localStorage.getItem("youtubeDigestPreview:ytd_settings"),
    null,
  );

  await storage.set({
    ytd_settings: { aiApiKey: "secret-ai-key", supadataApiKey: "secret-supadata-key" },
    ytd_notes: [{ text: "private note" }],
    digest_video123: { transcriptText: "private transcript" },
  ...
  assert.equal(localStorage.getItem("youtubeDigestPreview:ytd_notes"), null);
  assert.equal(
    localStorage.getItem("youtubeDigestPreview:digest_video123"),
    null,
  );
  assert.equal(
    localStorage.getItem("youtubeDigestPreview:ytd_options_language"),
    '"zh-CN"',
  );
  assert.equal(
    (await storage.get("ytd_settings")).ytd_settings.aiApiKey,
    "secret-ai-key",
  );
});

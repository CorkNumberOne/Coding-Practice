...
- Return a JSON object with exactly this shape: {"segments":[{"id":"unchanged-id","text":"translated text","source":"source text with restored punctuation"}]}.
- Copy every input id exactly. Translate only the `text` values. Only restore
  punctuation and capitalization in the `source` values.

...
test("Overview shares the Transcript batch generation and retries when opened", () => {
  ...
  assert.match(js, /const TRANSLATION_BATCH_SIZE = 4/);
  assert.match(
    js,
...
test("alignment batches return player units without repeating the full translation", async () => {
  const requests = [];
  const background = loadBackgroundHelpers({
  ...
  const result = await background.handleTranslateContent(
    {
      segments: [
        {
          id: "segment-0-0",
          ...
          displayUnits: [
            { id: "player-1::segment-0-0", text: "First sentence." },
            { id: "player-2::segment-0-0", text: "Second sentence." },
          ],
        },
      ],
    },
  ...
  assert.equal(result.success, true);
  assert.equal(result.translatedContent.segments[0].text, "");
  assert.deepEqual(
    JSON.parse(
      JSON.stringify(result.translatedContent.segments[0].displayUnits),
    ),
    [
      {
        id: "player-1::segment-0-0",
  ...
  assert.match(requests[0].messages[0].content, /Do not return the complete translation/);
});

test("translation keeps the original source and only returns translated text", () => {
  const background = loadBackgroundHelpers();
  const source = [
    {
      id: "segment-0-0",
  ...
  assert.equal(restored.segments[0].source, source[0].text);
...
test("background rejects unsupported language fallthrough and malformed batches", () => {
  ...
  assert.match(
    source,
    /\["transcriptBatch", "interfaceBatch", "alignmentBatch"\]/,
  );
  assert.throws(
...
test("Chinese prompt preserves natural bilingual-learning style rules", () => {
  ...
  assert.match(prompt, /do not merge, split, omit, or reorder segments/);
});

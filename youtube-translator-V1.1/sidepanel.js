...
let playerCaptionTranslationTimer = null;
let playerCaptionPrefetchInterval = null;
const pendingPlayerCaptionTranslations = new Map();
const playerCaptionParagraphCache = new Map();
const playerCaptionTranslationFailures = new Set();
let activePlayerCaptionTranslationQueue = null;
...
// ============================================================
// BILINGUAL PLAYER CAPTIONS
// ============================================================

function playerCaptionTranslationCacheKey(videoId, captionId) {
  return `${videoId}:zh:player:${captionId}`;
}
...
/**
 * Player captions need a much shorter reading span than transcript rows.
 * Sentence and clause boundaries remain preferred, with a hard character cap.
 */
function splitPlayerCaptionText(text, limits = PLAYER_CAPTION_LIMITS) {
  const normalized = normalizeCaptionText(text);
  if (!normalized) return [];

  const maxChars = getPlayerCaptionMaxChars(normalized, limits);
  const sentenceParts =
    normalized.match(/[^.!?;:,。！？；：，]+(?:[.!?;:,。！？；：，]+["')\]”’）】」』]*|$)/g) ||
    [normalized];
  const chunks = [];
  ...
  const flush = () => {
    const chunk = normalizeCaptionText(current);
    if (chunk) chunks.push(chunk);
    current = "";
  };

  sentenceParts.forEach((sentencePart) => {
    splitOversizedThought(sentencePart, maxChars).forEach((piece) => {
      if (!piece) return;

      if (!current) {
        current = piece;
      } else {
        const candidate = normalizeCaptionText(`${current} ${piece}`);
      ...
      const clauseEnd =
        /[;:,；：，]["')\]”’）】」』]*$/.test(piece);
      if (
        (semanticEnd && current.length >= limits.minChars) ||
        (clauseEnd && current.length >= limits.idealChars)
      ) {
        flush();
      }
    });
...
function splitStrictSentenceParts(text) {
  const normalized = normalizeCaptionText(text);
  if (!normalized) return [];
...
function getPlayerSentenceWordCount(text) {
  const source = String(text || "");
  const latinWords =
    source.match(/[A-Za-z]+(?:['-][A-Za-z]+)*/g) || [];
  const cjkCharacters = source.match(/[\u3400-\u9fff]/g) || [];
  return latinWords.length + cjkCharacters.length;
}

function splitPlayerCaptionByLimit(
  text,
  limits = PLAYER_CAPTION_LIMITS,
) {
  const normalized = normalizeCaptionText(text);
  if (!normalized) return [];
  const maxChars = getPlayerCaptionMaxChars(normalized, limits);
  ...
  const flush = () => {
    const value = normalizeCaptionText(current);
  ...
  clauses.forEach((clause) => {
    const clauseParts =
      normalizeCaptionText(clause).length > maxChars
        ? splitOversizedThought(clause, maxChars)
        : [normalizeCaptionText(clause)];
    clauseParts.forEach((part) => {
      if (!part) return;
      const candidate = normalizeCaptionText(`${current} ${part}`);
      const candidateLimit = getPlayerCaptionMaxChars(candidate, limits);
      if (current && candidate.length > candidateLimit) flush();
      current = normalizeCaptionText(`${current} ${part}`);
      if (current.length >= getPlayerCaptionMaxChars(current, limits)) {
        flush();
      }
    });
  });
...
function buildPlayerSentenceUnits(entries) {
  const pieces = [];
  ...
  entries.forEach((entry) => {
    ...
    const sentenceParts = splitStrictSentenceParts(text);
    const totalChars = sentenceParts.reduce(
    ...
    sentenceParts.forEach((part) => {
      const startRatio = totalChars ? consumedChars / totalChars : 0;
      consumedChars += part.length;
      const endRatio = totalChars ? consumedChars / totalChars : 1;
      const partStart = start + duration * startRatio;
      const partEnd = start + duration * endRatio;
      pieces.push({
        text: part,
        start: partStart,
        end:
...
function buildPlayerCaptionDisplayUnits(sentences, limits = PLAYER_CAPTION_LIMITS) {
  const units = [];

  for (let index = 0; index < sentences.length; index += 1) {
    const sentence = sentences[index];
    const nextSentence = sentences[index + 1];
    const sentenceWordCount = getPlayerSentenceWordCount(sentence.text);

    if (
      sentenceWordCount < 6 &&
      nextSentence
    ) {
      const combinedText = normalizeCaptionText(
        `${sentence.text} ${nextSentence.text}`,
      );
      if (
        combinedText.length <= getPlayerCaptionMaxChars(combinedText, limits)
      ) {
        units.push({
          start: sentence.start,
          end: Math.max(sentence.end, nextSentence.end),
          timingEstimated: sentence.timingEstimated,
          text: combinedText,
        });
        index += 1;
        continue;
    ...
    splitPlayerCaptionByLimit(sentence.text, limits).forEach(
      (part, partIndex, parts) => {
        const duration = Math.max(0.05, sentence.end - sentence.start);
        const totalChars = parts.reduce(
          (sum, value) => sum + value.length,
          0,
        );
        const precedingChars = parts
          .slice(0, partIndex)
          .reduce((sum, value) => sum + value.length, 0);
        const partStart =
          sentence.start + (duration * precedingChars) / totalChars;
        const partEnd =
          sentence.start +
          (duration * (precedingChars + part.length)) / totalChars;
        units.push({
          start: partStart,
          end: Math.max(partStart + 0.05, partEnd),
          timingEstimated:
            sentence.timingEstimated || partIndex > 0,
          text: part,
        });
      },
    );
...
function groupPlayerCaptionEntries(entries) {
  const sentences = buildPlayerSentenceUnits(entries);
  return buildPlayerCaptionDisplayUnits(sentences).map((unit, index) => ({
    id: `player-sentence-${index}-${Math.round(unit.start * 1000)}`,
    start: unit.start,
    end: unit.end,
    timingEstimated: unit.timingEstimated,
    text: unit.text,
  }));
}

function buildPlayerBilingualCaptionSegments(entries) {
  const groups = [];
  let current = null;
  ...
  entries.forEach((entry, index) => {
    ...
    if (!current) {
      ...
      const limit = getPlayerCaptionMaxChars(candidate);
    ...
    if (
      atSentenceEnd ||
      current.text.length >= getPlayerCaptionMaxChars(current.text)
    ) {
      flush();
    }
  });
...
function getPlayerBilingualCaptionSegments() {
  return buildPlayerBilingualCaptionSegments(getPlayerCaptionSourceEntries());
}

function getPlayerBilingualCaptionSources() {
  return getPlayerBilingualCaptionSegments().map((caption) => ({
    id: caption.id,
    start: caption.start,
    end: caption.end,
    timingEstimated: caption.timingEstimated,
    text: caption.text,
    translation:
      playerCaptionParagraphCache.get(
        playerCaptionTranslationCacheKey(currentVideoId, caption.id),
      ) || "",
  }));
}
...
async function syncPlayerBilingualCaptions({
  clear = false,
  force = false,
} = {}) {
  if (
    ...
    clearTimeout(playerCaptionTranslationTimer);
    playerCaptionTranslationTimer = null;
    pendingPlayerCaptionTranslations.clear();
    activePlayerCaptionTranslationQueue = null;
    if (clear) playerCaptionTranslationFailures.clear();
    stopPlayerCaptionTranslationPrefetch();
    await sendPlayerCaptionMessage({ action: "clearPlayerBilingualCaptions" });
    return;
  }

  const captions = getPlayerBilingualCaptionSources();
  if (!captions.length) return;

  const signature = `${currentVideoId || ""}:${captions.length}:${captions[0].id}:${captions.at(-1).id}`;
...
function queuePlayerCaptionTranslation(segment, translation, source = "") {
  if (
    currentTranscriptMode !== "bilingual" ||
    !segment?.id ||
    !translation
  ...
  pendingPlayerCaptionTranslations.set(segment.id, {
    translation,
    source,
  });
  clearTimeout(playerCaptionTranslationTimer);
  playerCaptionTranslationTimer = setTimeout(() => {
    playerCaptionTranslationTimer = null;
...
async function requestPlayerCaptionTranslationBatch(
  indices,
  segments,
  generation,
  videoId,
  ...
  try {
    ...
    aligned.forEach((item, batchIndex) => {
      ...
      const cacheKey = playerCaptionTranslationCacheKey(videoId, segment.id);
      if (!item.text) {
        playerCaptionTranslationFailures.add(cacheKey);
        return;
      }

      playerCaptionParagraphCache.set(cacheKey, item.text);
      queuePlayerCaptionTranslation(segment, item.text);
    });
    await updateCache();
  } catch {
    sourceBatch.forEach((segment) => {
      playerCaptionTranslationFailures.add(
        playerCaptionTranslationCacheKey(videoId, segment.id),
      );
    });
  } finally {
...
function createPlayerCaptionTranslationQueue(
  segments,
  generation,
  videoId,
  mode,
  ...
  const processNext = async () => {
    ...
    try {
      await requestPlayerCaptionTranslationBatch(
        indices,
        segments,
        generation,
        videoId,
  ...
  const enqueue = (index, force = false) => {
    ...
    if (
      ...
      queued.has(index) ||
      (!force &&
        (playerCaptionParagraphCache.has(cacheKey) ||
          playerCaptionTranslationFailures.has(cacheKey)))
    ) {
      return;
    }
  ...
  return { enqueue, generation, videoId, mode };
}

function getOrCreatePlayerCaptionTranslationQueue(
  segments,
  generation,
  videoId,
  mode,
  ...
  if (
    activePlayerCaptionTranslationQueue?.generation === generation &&
    activePlayerCaptionTranslationQueue.videoId === videoId &&
    activePlayerCaptionTranslationQueue.mode === mode
  ) {
    return activePlayerCaptionTranslationQueue;
  }

  activePlayerCaptionTranslationQueue = createPlayerCaptionTranslationQueue(
    segments,
    generation,
    videoId,
    mode,
...
async function prefetchPlayerCaptionTranslations() {
  ...
  try {
    ...
    const playerSegments = getPlayerBilingualCaptionSegments();
    ...
    const playerQueue = getOrCreatePlayerCaptionTranslationQueue(
      playerSegments,
...
async function startDigest(videoId, videoUrl) {
  ...
  if (cached) {
    ...
    if (cached.playerCaptionCache) {
      for (const [key, value] of Object.entries(cached.playerCaptionCache)) {
        playerCaptionParagraphCache.set(key, value);
      }
    }
...
function showState(state) {
  ...
  document.getElementById("captionScaleControl").style.display =
    state === "results" ? "inline-flex" : "none";
...
async function saveToCache(videoId) {
  ...
  try {
    ...
    const playerCaptionCacheForVideo = {};
    for (const [key, value] of playerCaptionParagraphCache.entries()) {
      if (key.startsWith(`${videoId}:`)) {
        playerCaptionCacheForVideo[key] = value;
      }
    }
    ...
    const cacheData = {
      ...
      playerCaptionCache: playerCaptionCacheForVideo,
      timestamp: Date.now(),
    };
...
async function handleDisplayLanguageModeChange(mode) {
  ...
  if (mode === "bilingual") playerCaptionTranslationFailures.clear();
  translationGeneration += 1;
...
function createTranscriptTranslationQueue(
  ...
  const enqueue = (index, force = false) => {
    ...
    if ((!force && translationCached && sourceReady) || queued.has(index)) {
      return;
    }
    queue.push(index);
    queued.add(index);

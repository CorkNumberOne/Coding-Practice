const YTD_OPTIONS = (() => {
  ...
  const COPY = {
    en: {
      ...
      footer:
        'Read <a href="PRIVACY.md" target="_blank" rel="noreferrer">PRIVACY.md</a> in the repository for the complete data-flow description.',
      migrationWarning:
    ...
    "zh-CN": {
      ...
      resetData: "重置扩展数据",
      footer:
        '完整数据流说明请参阅仓库中的 <a href="PRIVACY.md" target="_blank" rel="noreferrer">PRIVACY.md</a>。',
      migrationWarning:
  ...
  function createStorageAdapter(
    ...
    function readFallbackValue(key) {
      if (isSensitiveKey(key)) return memoryStorage.get(key);
      try {
    ...
    for (const key of fallbackKeys()) {
      const logicalKey = key.slice(PREVIEW_STORAGE_PREFIX.length);
      if (!isSensitiveKey(logicalKey)) continue;
      try {
        fallbackStorage?.removeItem(key);
      } catch (_error) {
        // A locked-down preview may reject storage cleanup.
      }
    }
